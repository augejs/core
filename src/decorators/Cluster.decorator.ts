...
import { ScanNode, Metadata } from '@augejs/provider-scanner';
import {
  LifecycleOnAppWillCloseHook,
  Lifecycle__onAppReady__Hook,
} from './LifecycleHook.decorator';
...
Cluster.getMetadata = (target: NewableFunction): ClusterOptions => {
  return (
    (Metadata.getMetadata(Cluster, target) as ClusterOptions) || {
      workers: 0,
    }
  );
};
...
Cluster.ClusterMasterClassDecorator = function (
  opts?: ClusterOptions,
): ClassDecorator {
  opts = {
    workers: 0,
      ...
      [
        Lifecycle__onAppReady__Hook(
          async (scanNode: ScanNode, next: CallableFunction) => {
            ...
            let workers: number =
              opts?.workers || parseInt(process.env.WORKERS || '0') || 0;
            if (workers < 0) {
                  ...
                  (resolve: CallableFunction, reject: CallableFunction) => {
                    const worker = cluster
                      ...
                      .once('exit', (code: number, signal: string) => {
                        reject(
                          new Error(
                            `worker(pid: ${worker.process.pid}) code: ${code} signal: ${signal} Boot Error`,
                          ),
                        );
                      })
            ...
            cluster.on(
              ...
              (worker: Worker, code: number, signal: string) => {
                if (!worker.exitedAfterDisconnect) {
                  logger.info(
                    `worker(pid: [${worker.process.pid}]) is exit. code: ${code} signal: ${signal} and will be forked new process`,
                  );
                  cluster.fork();
                }
              },
        ...
        LifecycleOnAppWillCloseHook(
          async (scanNode: ScanNode, next: CallableFunction) => {
            ...
            await Promise.all(
              Object.values(cluster.workers).map((worker?: Worker) => {
                return new Promise((resolve: CallableFunction) => {
                  ...
                  worker.once('exit', () => {
                    logger.warn(
                      `worker(pid: [${worker.process.pid}]) is exited with signal: SIGTERM`,
                    );
                    resolve();
                  });
                  ...
                  logger.info(
                    `worker(pid: [${worker.process.pid}]) is killed with signal: SIGTERM`,
                  );
                });
              }),
            );
